...
BEGIN
	...
	sqlcmd := sqlcmd || ') TO ''${PASTA_SAIDA}${CLIENTE}\mudancastrabalhadores.csv'' WITH CSV HEADER DELIMITER '';'' ENCODING ''WIN1252'';';
...
BEGIN
	...
	COPY (SELECT DISTINCT ON (grupoempresarial, empresa, codigo) * FROM eventos ORDER BY grupoempresarial, empresa, codigo) TO '${PASTA_SAIDA}${CLIENTE}\rubricas.csv' WITH CSV HEADER DELIMITER ';' ENCODING 'WIN1252';
